...
//2~100까지 수로 나누어서 나머지가 0이면 소수 아님(false 반환)
for (let i = 2; i < 100; i++) {
  for (let j = 2; j < i; j++) {
    if (i % j == 0) {
      isPrime = false;
...
for (let n of prime) {
  //prime에 모여 있는 수들은 모두 소수. 소수 모음을 하나씩 탐색
  console.log("n: " + n);
  console.log("val - n: " + (val - n));
  //prime에 val - n이 포함되어 있고 val - n이 val - n 이상이라면
  if (prime.includes(val - n) && n <= val - n) {
    //골드바흐파티션에 n과 val - n 값 push
    골드바흐파티션.push([n, val - n]);
  }
}
//이중배열에서 안쪽 배열의 [1]와 [0] 빼기
